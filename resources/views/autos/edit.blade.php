                            ...
                            <div class="container">
                                <form action="{{ route('autos.update', $auto) }}" method="POST">
                                    ...
                                    <input id="placa" class="col-12 inputborder form-control" type="text"
                                        name="placa" value="{{ $auto->placa }}">
                                    <br>
                                    <label class="col" for="">Cliente:</label>
                                    <br>
                                    <input type="text" value="{{ $auto->cliente }}"
                                        class="col-12 inputborder form-control" disabled>
                                    <select id="cliente" class="col-12 inputborder form-control" name="cliente">
                                        @foreach ($clientes as $cliente)
                                            <option value="{{ $cliente->nombre }} {{ $cliente->app }} {{ $cliente->apm }}">
                                                {{ $cliente->nombre }} {{ $cliente->app }} {{ $cliente->apm }}</option>
                                        @endforeach
                                    </select>
                                    <br>
